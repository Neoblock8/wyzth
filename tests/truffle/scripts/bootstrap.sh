...
function generate_genesis() {
     ...
     node generate-genesis.js --chainid ${WYZTH_CHAIN_ID} --wyzthChainId ${chainIDHex}
}
...
function init_genesis_data() {
     ...
     sed -i -e "s/{{NetworkId}}/${WYZTH_CHAIN_ID}/g" ${workspace}/storage/${node_id}/config.toml
     if [ "${node_id}" == "wyzth-rpc" ]; then
...
for((i=1;i<=${NUMS_OF_VALIDATOR};i++)); do
     init_validator "wyzth-validator${i}"
done
...
init_genesis_data wyzth-rpc wyzth-rpc

for((i=1;i<=${NUMS_OF_VALIDATOR};i++)); do
     init_genesis_data validator "wyzth-validator${i}"
done
